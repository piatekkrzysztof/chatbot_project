...
Data rozpoczęcia: 9.09.2026. **Aktualizacja: 13.09.2026. Backend #56 (F25, 2.0.15), #57 (F17, 2.0.16), #58 (F10 cz. 1, 2.0.17), #59 (F10 cz. 2, 2.0.18) #60 (F19, 2.0.19) i #61 (pomiar wariantów promptu, 2.0.20) scalone, `/health/` produkcji zwraca 2.0.20; `/health/` produkcji zwracało 2.0.15 przy ostatnim sprawdzeniu. Bieżący etap 2.0.21: poprawka promptu F25 - przypomnienie o znaczniku, wybrane pomiarem wariantów - przygotowane do przeglądu. Pełna kopia i próba odtworzenia (2.0.14) nadal czekają na odbiór operacyjny, więc F18 nie włącza automatycznego usuwania.**
...
| 5. Wiedza i cykl życia danych | F10, F17, F18, F19, F25 | Kompletny import, atomowa publikacja embeddingów i usuwanie pochodnych, poprawne CSV i feedback, wyszukiwanie FAQ i regresja RAG | F25: #56 scalony, produkcja zwraca 2.0.15; do potwierdzenia pomiar `ocen_generowanie`. F17: #57 scalony (2.0.16). F10 część 1: #58 scalony (2.0.17); część 2: #59 scalony (2.0.18). F19: #60 scalony (2.0.19). Do wykonania: F18; F25 - poprawka promptu (2.0.21) przygotowana do przeglądu |
| 6. Płatności | F11; status płatności z F22 | Idempotencja Checkout/webhooków, identyfikatory i okresy Stripe, retry/uzgadnianie; UI potwierdza konkretny zakup | F11 część 1 przygotowana do przeglądu (2.0.22): webhook synchronizuje stan subskrypcji ze Stripe, blokada drugiego zakupu, klucz idempotencji Checkout. Do wykonania: część 2 - zmiana planu na tej samej subskrypcji i potwierdzenie konkretnego zakupu w panelu |
...
| F04 | Prywatne magazyny, szyfrowanie i klucze poza hostingiem; 2.0.14 dodaje pełne kopie z bajtami plików i zweryfikowaną próbę na danych syntetycznych | Wdrożenie narzędzi, rzeczywista kopia przy wstrzymanych zapisach, izolowany restore, harmonogram i alarmy |
...
| F11 | Część 1 przygotowana do przeglądu (2.0.22): stan subskrypcji pobierany ze Stripe zamiast wykonywania zdarzeń po kolei, identyfikator subskrypcji przy firmie, okres i plan ze Stripe, dostęp do końca opłaconego okresu + 3 dni przy nieudanym odnowieniu, 500 przy przejściowym błędzie, blokada drugiego zakupu, klucz idempotencji | Przegląd i CI, migracja, włączenie `customer.subscription.updated` w webhooku Stripe, odbiór w trybie testowym Stripe według [platnosci-spojnosc.md](platnosci-spojnosc.md); część 2: zmiana planu, potwierdzenie konkretnej sesji w panelu |
...
| F21 | Kontrola kopii #41, dostępny PITR; 2.0.14: zaszyfrowany pełny format, próba odtworzenia bazy i plików na danych syntetycznych | Wdrożenie i rzeczywisty restore na PG16, zmierzone RPO/RTO, działające harmonogramy/alarmy także przy braku przebiegów |
...
| F25 | #56 scalony (2.0.15): FAQ wybierane po dopasowaniu do pytania, treść klienta między ogranicznikami, wycięcie tokenów protokołu z treści; regresja RAG już w CI | Pomiar 13.09.2026 na produkcji: regresja znacznika (odmowy trafne 91,7%, oparte na wiedzy 90% przy oczekiwanych 100%) - model odmawia słowami bez `[BRAK_ODPOWIEDZI]`. Pomiar wariantów (13.09.2026): przyczyną zdanie o danych, ograniczniki bez wpływu. Poprawka: przypomnienie o znaczniku na końcu promptu (2.0.21); po wdrożeniu `ocen_generowanie --powtorzen 5`; szczegóły w [faq-i-rozdzielenie-tresci.md](faq-i-rozdzielenie-tresci.md) |
