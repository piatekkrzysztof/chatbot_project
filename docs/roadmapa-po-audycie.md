...
Data rozpoczęcia: 9.09.2026. **Aktualizacja: 13.09.2026. Backend #56 (F25, 2.0.15), #57 (F17, 2.0.16) i #58 (F10 cz. 1, 2.0.17) scalone; `/health/` produkcji zwracało 2.0.15 przy ostatnim sprawdzeniu. Bieżący etap 2.0.18 (#59): F10 część 2, limit wiedzy przy równoległych dodaniach oraz pliki TXT i DOCX - przygotowane do przeglądu, jeszcze niewdrożone. Pełna kopia i próba odtworzenia (2.0.14) nadal czekają na odbiór operacyjny, więc F18 nie włącza automatycznego usuwania.**
...
| F10 | Część 1: #58 scalony (2.0.17): typ treści przy pobieraniu stron, PDF/DOCX/TXT/MD podlinkowane na stronie przez izolowany parser, adres źródła zawsze pobierany, mapy stron stałych przed wpisami, jedna za duża odpowiedź nie przerywa pobierania. Część 2: #59 przygotowany do przeglądu (2.0.18): limit bazy wiedzy pod blokadą doradczą, zlecenia zadań po zatwierdzeniu transakcji, TXT w Windows-1250, ISO-8859-2 i UTF-16, tabele i pola tekstowe DOCX | Przegląd i CI części 2, wdrożenie web i workera, kontrole z [kompletny-import.md](kompletny-import.md) |
...
| F25 | #56 scalony (2.0.15): FAQ wybierane po dopasowaniu do pytania, treść klienta między ogranicznikami, wycięcie tokenów protokołu z treści; regresja RAG już w CI | Pomiar `ocen_generowanie --powtorzen 3` na prawdziwym modelu (lokalnie niewykonalny: przechwytywanie TLS) i zapis wyniku; potwierdzenie, że worker też działa na 2.0.15 |
