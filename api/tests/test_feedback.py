...
def test_submit_valid_feedback(user, tenant, subscribtion):
    ...
    res = client.post(
        "/api/chat/feedback/",
        {"message_id": message.id, "is_helpful": True},
...
def test_feedback_missing_message_id(user, tenant, subscribtion):
    ...
    client.force_authenticate(user=user)
    res = client.post(
        "/api/chat/feedback/", {"is_helpful": True}, HTTP_X_API_KEY=str(tenant.api_key)
    )
...
def test_feedback_missing_is_helpful(user, tenant, subscribtion):
    ...
    res = client.post(
        "/api/chat/feedback/", {"message_id": msg.id}, HTTP_X_API_KEY=str(tenant.api_key)
    )
    assert res.status_code == 400
...
def test_feedback_nonexistent_message(user, tenant, subscribtion):
    ...
    res = client.post(
        "/api/chat/feedback/",
        {"message_id": 999, "is_helpful": True},
        HTTP_X_API_KEY=str(tenant.api_key),
    )
    assert res.status_code == 400
    assert "message_id" in res.json()
...
def test_feedback_rejected_for_user_message(user, tenant, subscribtion):
    ...
    res = client.post(
        "/api/chat/feedback/",
        {"message_id": user_msg.id, "is_helpful": True},
        HTTP_X_API_KEY=str(tenant.api_key),
    )
    assert res.status_code == 400
    assert "message_id" in res.json()
...
def test_feedback_overwrites_previous(user, tenant, subscribtion):
    ...
    res = client.post(
        "/api/chat/feedback/",
        {"message_id": msg.id, "is_helpful": True},
        HTTP_X_API_KEY=str(tenant.api_key),
    )
    assert res.status_code == 200
